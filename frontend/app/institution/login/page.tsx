...
function Page() {
  ...
  const onSubmit = async (data: LoginForm) => {
    try {
    const res = await axiosInstance.post('/institution/login', data)
    toast.success(res.data.message)
    const token =  res.data.token
    console.log("->",token)
    dispatch(tokenSlice.actions.saveToken(token))
    localStorage.setItem("institution-token", res.data.token);
    reset()
      ...
      <div className="w-full h-full grid grid-cols-1 md:grid-cols-2">
        ...
        <div className="flex flex-col justify-center px-6 sm:px-10 md:px-14 lg:px-20 bg-background">
          ...
          <p className="text-sm sm:text-base lg:text-lg text-text-secondary mb-10 max-w-md">
            Secure access to manage your students platform and adding courses
          </p>
